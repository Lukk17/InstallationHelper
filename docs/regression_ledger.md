...
Current as of 2026-08-15, repository HEAD e407d4f29cb6aa43c1e1ca84e78ed389ae36823f.
...
A harness under [e2e/](../e2e/) mechanises part of this checklist. Items 4 and 5 are fully covered by its tier 1 checks, which run in seconds and are proven to fail against the code from before each fix. Item 2 is covered by tier 2 for Arch, the Arch User Repository, Flathub, Homebrew, Chocolatey and winget over HTTP, and for apt and dnf inside the same pinned base images the tier 3 scenarios use, with the names that only exist after the playbook adds a repository listed in [runtime_repo_packages.txt](../e2e/tier2/runtime_repo_packages.txt) rather than silently forgiven. Items 1 and 6 are covered by tier 3, which runs the real playbook in a container. Debian, Ubuntu and Fedora images exist and are built, but only Arch has been run end to end so far, so for the other three families items 1 and 6 are still something a person has to do by hand. Items 3, 7, 8, 9 and 10 are not mechanised at all and remain review discipline.
