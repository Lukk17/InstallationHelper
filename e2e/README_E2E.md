...
On Windows, tier 1 is best run from Git Bash. Neither Windows shell can prove everything on its own: WSL has Ansible but the only PowerShell 7 installed here is the Microsoft Store build, whose real executable sits under `C:\Program Files\WindowsApps` where WSL can neither see nor execute it, and Git Bash has a working `pwsh` but no Ansible. Git Bash is the one that closes the gap, because the two checks that need Ansible, `tier1/ansible_static.sh` and `tier1/os_family_derivation.sh`, re-execute themselves inside WSL when `ansible-playbook` is missing locally, through the helper they share, `tier1/wsl_delegate.sh`. From Git Bash the gate reports every check as passed. From WSL it reports one fewer pass and an honest SKIP on the PowerShell mapping comparison. The absolute number is not written down here on purpose, because it changes with every check added and a stale total in prose is read as the definition of the gate's size.
