...
On Windows, tier 1 passes from Git Bash and, as measured on 2026-09-02, from WSL as well, with the same tally from both and the same two skips. WSL is new to that list, and until that day it was quietly running about four fifths of the gate. Fourteen checks resolved their interpreter as `${PYTHON:-python}`, WSL Ubuntu here has `/usr/bin/python3` and no `python`, and `set -e` ended each of those scripts on the assignment: twelve printed a header and nothing else, a thirteenth got one assertion out first, and `shell_syntax` turned the same failure into `FAIL no shell script was found anywhere in the repository`, which is an accusation about the tree rather than about the shell. Two more, `windows_pester` and `powershell_variables`, converted their paths with `host_path`, which is the Docker converter and the identity on Linux, and so handed a Windows `pwsh.exe` a `/mnt/d` path it could only answer `Cannot find path 'D:\mnt\d\...'` to. The interpreter is resolved once now, by `require_python` in [lib/common.sh](lib/common.sh), which reports SKIP and names what it looked for when nothing answers, and those two checks use `to_windows_path`. The failure this section used to record, `setup/manifests/generate.py` calling `Path.read_text(newline="")` against WSL's Python 3.12.3, is gone too, because the generator compares bytes. Each shell reaches what it is missing a different way. Git Bash has `pwsh` and no Ansible, so the two checks that need Ansible, `tier1/ansible_static.sh` and `tier1/os_family_derivation.sh`, re-execute themselves inside WSL when `ansible-playbook` is missing locally, through the helper they share, `tier1/wsl_delegate.sh`. WSL has Ansible and no native PowerShell, and reaches the Windows `pwsh.exe` through interop, which was measured rather than assumed: the only PowerShell 7 here is the Microsoft Store build, whose `WindowsApps` entry is an app-execution alias, and it does run from WSL. That is why `tier1/pwsh_probe.sh` decides by running a candidate rather than by finding it on PATH, and why it still reports SKIP rather than a pass when nothing runs. Whether an alias works from WSL is a property of the Windows build, so treat it as something to re-measure rather than as a fact this document owns. The absolute number of checks is not written down here on purpose, because it changes with every check added and a stale total in prose is read as the definition of the gate's size.
...
| [tier1/sigpipe_pipelines.sh](tier1/sigpipe_pipelines.sh) | A command fed from `yes`, which the kernel kills with SIGPIPE when the reader exits, so under `set -o pipefail` the pipeline reports 141 whatever the real command did. It happened to the Android SDK licence task, was fixed there, and was then written again into the CachyOS graphics alignment task, where it failed a run on a real machine on 2026-08-28 over a pacman that had succeeded |
| [tier1/unreachable_tasks.sh](tier1/unreachable_tasks.sh) | A task gated on a toggle the container limits switch off on every image that could run it, which is a task the entire suite never executes. That is how the CachyOS alignment task shipped untested: it was pinned to the one image where `install_steam` was suppressed. Deliberate cases are listed with what they cost in [tier1/uncovered_toggles_allowed.txt](tier1/uncovered_toggles_allowed.txt), and a stale line there fails too |
| [tier1/keycloak_realm_import.sh](tier1/keycloak_realm_import.sh) | The committed Keycloak realm export acquiring something Keycloak's own import path will not take, losing the client or the user the readme promises, or the image quietly ceasing to seed itself from it at all. A JavaScript authorization policy in that realm crash looped the container for four minutes on `Script upload is disabled` while the health check said `starting`, and `--import-realm` never touches an existing realm, so any machine whose database predates the fix still holds the policy and re-exports it |
| [tier1/desktop_settings.sh](tier1/desktop_settings.sh) | Anything here changing a screen lock, idle or session-restore setting on any platform. The konsave profile carried a `kscreenlockerrc` holding nothing but a version stanza, and applying it replaced rather than merged, so a machine set never to lock went back to Plasma's five minute default with no warning. The check reads inside the `.knsv` archive, which no grep over the tree would ever have found |
