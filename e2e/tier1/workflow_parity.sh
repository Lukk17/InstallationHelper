...
def capabilities(job):
    ...
    for step in job.get('steps') or []:
        ...
        body = step.get('run')
        if isinstance(body, str) and FREES_DISK.search(body):
            frees = True
    return uses, frees
...
for label, prefix in (('Windows', 'windows-'), ('macOS', 'macos-')):
    ...
    if problems:
        ...
        lines.append('OK %s: %d single-platform job(s) match the sweep on %d action(s) and on '
                     'freeing disk space' % (label, len(single), len(sweep_uses)))
