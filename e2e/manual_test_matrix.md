...
| CachyOS | Nothing distribution-specific any more. `install_steam` was suppressed here until 2026-08-28 on the grounds that the v3 index named `lib32-glibc` and `lib32-gcc-libs` builds the content delivery network answered 404 for. Re-measured, that was a misreading: those exact builds answer 200 on `mirror.cachyos.org` and 404 only on `cdn77.cachyos.org`, so the index is right and one mirror is incomplete. The suppression file is gone and steam is installed and verified on CachyOS like everywhere else | Covered by the container tier | `steam` appears in `pacman -Q`. If a run fails retrieving a `lib32-` file, read `/etc/pacman.d/cachyos-mirrorlist` before blaming the repository |
