...
Force a rebuild of the base image, and only when the Dockerfile for the distribution you are targeting changed since the last run, [../tier3/arch.Dockerfile](../tier3/arch.Dockerfile) by default and one of debian, ubuntu or fedora beside it. Do it before step 1, never between the two runs, so both scenarios test the same image.
...
11. `Assert every enabled outside-package-manager tool is installed and reachable from a plain interactive shell`. Both scenarios leave the ai_tools and sdk_manager toggles at their default of true, so this checks the whole set, node through the six npm-installed AI CLIs, in both the KDE run and the GNOME run. Same "Found on a real CachyOS machine on 2026-09-15" ledger entry as capability 60, and a place worth watching here specifically: the shell it probes is an interactive non-login shell of `{{ non_root_user }}`'s real login shell, not whichever a desktop environment role happens to assume.
