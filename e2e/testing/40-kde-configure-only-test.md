...
- The per-distribution group names still resolve on this path. `id -nG` must contain the group named by `os_dict.openrazer_device_group` and `docker`, both because those toggles are true by default. Same "Arch OpenRazer device group does not exist" ledger entry as capability 30.
