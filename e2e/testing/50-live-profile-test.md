...
10. `Assert every enabled outside-package-manager tool is installed and reachable from a plain interactive shell`. The profile turns off `install_nodejs`, `install_python`, `install_java`, `install_flutter`, `install_dart`, `install_android_sdk`, `install_bruno_cli` and every AI CLI toggle, which is every toggle this assertion looks at, so nothing is enabled for it to check and it passes on an empty set. That is a real answer rather than a gap: capability 60 is where this assertion is exercised against the full default set, including the "Found on a real CachyOS machine on 2026-09-15" ledger entry it exists for.
