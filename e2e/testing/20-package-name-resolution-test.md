...
Seven check results, one per index group. Arch official repositories, the Arch User Repository, Flathub, Homebrew formulae, Homebrew casks, Chocolatey and winget. Each pass states how many names resolved. Each failure states how many of how many do not exist and names them, so the output is a work list rather than a verdict.
