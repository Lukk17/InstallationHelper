...
# ansible-core is whatever CachyOS publishes, and it cannot be pinned to an exact version here:
# CachyOS is rolling and takes ansible-core straight from Arch's extra repository, so it carries
# exactly one version and the previous one is gone from every mirror the moment it is superseded. An
# exact pin would break the build rather than hold it steady. It built at 2.21.3 on 2026-08-20.
#
